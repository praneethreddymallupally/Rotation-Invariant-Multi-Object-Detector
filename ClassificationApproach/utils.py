...
def crop_largest_rectangle(image, angle, height, width):
    return crop_around_center(image,*largest_rotated_rect(width,height,math.radians(angle)))
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            if self.rotate:
                ...
                rotation_angle = 0
            rotated_image = generate_rotated_image(image,rotation_angle,size=self.input_shape[:2],crop_center=self.crop_center,
...
def test_my_model(model, input, num_images=5, size=None, crop_center=False,
    ...
    for rotated_image, true_angle, predicted_angle in zip(x_rot, y, y_pred):
        ...
        cv2.imwrite('/content/drive/MyDrive/MajorProjectBP1/RotNet-master - Copy/data/temp_test/image2.jpg', img)
        img = imgtool.load_img('/content/drive/MyDrive/MajorProjectBP1/RotNet-master - Copy/data/temp_test/image2.jpg', target_size = (224, 224))
        ...
        cv2.imwrite('/content/drive/MyDrive/MajorProjectBP1/RotNet-master - Copy/data/temp_test/image1.jpg', img)
        img = imgtool.load_img('/content/drive/MyDrive/MajorProjectBP1/RotNet-master - Copy/data/temp_test/image1.jpg', target_size = (224, 224))
        # # plt.imshow(img)
        img = imgtool.img_to_array(img)
        img = np.expand_dims(img, axis=0)
        img = preprocess_input(img)
...
def test_accuracies(model, input, num_images=5, size=None, crop_center=False,
    ...
    print("Prediction accuracy for original images: ",orginal_img_acc_sum/num_images)
    print("Prediction accuracy for rotated images: ",rotated_img_acc_sum/num_images)
    print("Prediction accuracy for corrected images: ",corrected_img_acc_sum/num_images)

def test_eigenVectors(model, input, num_images=5, size=None, crop_center=False,
                     crop_largest_rect=False, preprocess_func=None, save_path=None):
